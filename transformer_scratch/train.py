...
def  run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt,
                    max_len, device, print_msg, global_state, 
...
def train_model(config):
    ...
    lss_fn = nn.CrossEntropyLoss(ignore_index=tokenizer_tgt.token_to_id("[PAD]"), label_smoothing=0.1).to(device)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Run the tensors through thr tansformer
            ...
            loss = lss_fn(proj_output.view(-1, tokenizer_tgt.get_vocab_size()), label.view(-1))
