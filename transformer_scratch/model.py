...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # creat a avector of shape (seq_len, 1)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1) 
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * -(math.log(10000.0) / d_model))
        # Apple the sin to even posiotion
        pe[:, 0::2] = torch.sin(position * div_term)
        # Apple the cos to odd posiotion    
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__( self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_k = d_model // h
        self.w_q = nn.Linear(d_model, d_model)  # w_q and b_q
        self.w_k = nn.Linear(d_model, d_model)  # w_k and b_k
        self.w_v = nn.Linear(d_model, d_model)  # w_v and b_v
        self.w_o = nn.Linear(d_model, d_model)  # w_o and b_o
        self.dropout = nn.Dropout(dropout)
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, 
                      ...
                      h:int = 8, dropout = 0.1,d_ff: int = 2048)->Transformer:
    # Creat the embedding layers
    src_embed = InputEmbeddings(d_model, src_vocab_size)
    tgt_embed = InputEmbeddings(d_model, tgt_vocab_size)

    # Creat the positional encoding layers
